...
export default function PackageDetailSection({ navUrl, pkg }: { navUrl: string, pkg: any }) {
    ...
    return (
        <section className="mx-auto w-full px-4 py-4 sm:px-6 sm:py-10 lg:px-8 bg-linear-to-br from-emerald-50 via-white to-lime-50">
            <div className="mb-4 sm:mb-6 flex flex-wrap items-center justify-between gap-2 sm:gap-3">
                <button
                    onClick={() => { router.back() }}
                    className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/70 px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold text-emerald-600 transition-colors hover:bg-emerald-50 cursor-pointer">
                    <span className="text-sm sm:text-base">←</span>
                    <span>Back to packages</span>
                </button>
                <p className="rounded-full border border-emerald-100 bg-white px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-semibold uppercase tracking-widest sm:tracking-[0.18em] text-emerald-600">Trip details</p>
            </div>
            ...
            <div className="relative overflow-hidden rounded-3xl sm:rounded-4xl border border-emerald-100 bg-slate-900 shadow-[0_20px_60px_rgba(15,122,75,0.18)] sm:shadow-[0_28px_90px_rgba(15,122,75,0.22)]">
                <img src={`${API_BASE_URL}/${pkg?.photoUrls[0]}`}
                    className="w-full sm:h-80 md:h-96 lg:h-136 object-cover opacity-80" />
            ...
            <div className="mt-6 sm:mt-8 grid gap-5 sm:gap-8 grid-cols-1 lg:grid-cols-[1.2fr_0.8fr]">
                <div className="space-y-5 sm:space-y-6">
                    <article className="rounded-2xl sm:rounded-4xl border border-emerald-100 bg-white p-4 sm:p-7 shadow-md shadow-emerald-950/5">
                        <p className="text-xs font-semibold uppercase tracking-widest text-emerald-700">Overview</p>
                        <h2 className="mt-2 sm:mt-3 text-lg sm:text-2xl font-black text-slate-950">What this trip feels like</h2>
                        <p className="mt-3 sm:mt-4 text-sm sm:text-base leading-6 sm:leading-8 text-slate-600">{pkg?.intro}</p>
                    </article>
                ...
                <aside className="lg:sticky lg:top-24 lg:self-start">
                    <div className="rounded-4xl border border-emerald-100 bg-white p-7 shadow-lg shadow-emerald-950/10">
                        ...
                        <Link
                            href={`${navUrl}packages/${pkg?.slug}/booking`}
            ...
            <div className="mt-16 rounded-4xl border border-emerald-100 bg-white p-6 sm:p-8 lg:p-10 shadow-lg shadow-emerald-950/5">
                ...
                <div className="relative space-y-6 before:absolute before:left-4 before:top-2 before:h-[calc(100%-1rem)] before:w-px before:bg-emerald-200 sm:before:left-5">
                    {pkg?.itinerary?.map((day: any, index: number) => (
                        <article key={index} className="relative rounded-3xl border border-emerald-100 bg-emerald-50/35 p-5 sm:p-6">
